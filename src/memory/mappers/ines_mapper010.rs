...
pub struct InesMapper010 {
    ...
    fn _update_latch(&self, addr: u16) {
        match addr {
            0x0FDC => self.latch_0.set(0xFD),
            0x0FEC => self.latch_0.set(0xFE),
            0x1FDC..=0x1FDF => self.latch_1.set(0xFD),
            0x1FEC..=0x1FEF => self.latch_1.set(0xFE),
            _ => {}
        }
    }
}
...
impl Mapper for InesMapper010 {
    fn read(&self, addr: u16) -> u8 {
        match addr {
            ...
            0x8000..=0xBFFF => {
                ...
                let bank_offset = (self.banks[0] as usize) * 0x4000;
                let rom_offset = (addr - 0x8000) as usize;
                self.prg_rom[bank_offset + rom_offset]
            }
            0xC000..=0xFFFF => {
                //16kb fixed to the last 3 banks
                let start_of_last_16kb = self.prg_rom.len() - 0x4000;
                let rom_offset = (addr - 0xC000) as usize;
                self.prg_rom[start_of_last_16kb + rom_offset]
            }
            _ => { 0 }
        }
    }
    ...
    fn write(&mut self, addr: u16, val: u8) {
        match addr {
            ...
            0xA000..=0xAFFF => {
                self.banks[0] = val & 0b0001_1111
            }

            // 3F mask instead of the mmc2's 1F mask, which makes the maximum possible bank count 64 instead of 32
            0xB000..=0xBFFF => self.banks[1] = val & 0x3F, // Select 4 KB CHR ROM bank for PPU $0000-$0FFF (Latch 0 = $FD)
            0xC000..=0xCFFF => self.banks[2] = val & 0x3F, // Select 4 KB CHR ROM bank for PPU $0000-$0FFF (Latch 0 = $FE)
            0xD000..=0xDFFF => self.banks[3] = val & 0x3F, // Select 4 KB CHR ROM bank for PPU $1000-$1FFF (Latch 1 = $FD)
            0xE000..=0xEFFF => self.banks[4] = val & 0x3F, // Select 4 KB CHR ROM bank for PPU $1000-$1FFF (Latch 1 = $FE)
            
            //Mirroring ($F000-$FFFF)
            0xF000..=0xFFFF => {
                //opposite of the MMC2
