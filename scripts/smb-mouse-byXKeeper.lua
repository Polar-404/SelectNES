...
function hitbox(mx, my, _, _, x1, y1, x2, y2)
    return mx >= x1 and mx <= x2 and my >= y1 and my <= y2
end
function AND(val, mask) 
    return val 
end
...
function modechange(args)
	...
	if args[1] == 0 then
		...
		mainmenu['menu']['m002_enemy']['marked']	= 0;
	else
		mainmenu['menu']['m001_mode']['menu']['m001_tiles']['marked']	= 0;
		mainmenu['menu']['m002_enemy']['marked']	= 1;
	end;
end;
...
--						memory.writebyte(0x000E + i, 0x00);
	...
	last.xmouse = inpt.xmouse
	last.ymouse = inpt.ymouse
	last.leftclick = inpt.leftclick
end